...
function PosterArea({ entry, posterUrl, onToggle }) {
  const [imgError,  setImgError]  = useState(false)
  const [imgLoaded, setImgLoaded] = useState(false)
  const showReal = !!posterUrl && !imgError

  return (
    // Toggle only fires when clicking the poster itself
    <div
...
export default function EntryCard({ entry, onToggle, posterUrl, index = 0 }) {
  ...
  return (
    // No onClick here — toggle is handled inside PosterArea only
    <motion.div
    ...
    >
      <div className={`relative rounded-xl overflow-hidden transition-all duration-250 ${
        ...
        <PosterArea entry={entry} posterUrl={posterUrl} onToggle={onToggle} />
      </div>

      {/* Info below poster */}
      <div className="mt-2.5 px-0.5">
        <div className="text-white text-xs font-heading font-semibold uppercase tracking-wide leading-tight line-clamp-2">
          {entry.title}
        </div>
        ...
        {/* Watch on TV — always visible, never interferes with toggle */}
        <div className="mt-2">
